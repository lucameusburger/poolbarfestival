  ...
  return (
    <View style={StylesMain.mainView}>
      <FadeInView style={{ flex: 1, width: '100%', height: '100%' }}>
        ...
        />
        <View style={{ flex: 1, margin: 0 }}>{events ? <FlatList style={{ flex: 1, padding: 0 }} data={events} keyExtractor={(item) => item.id} renderItem={({ item }) => <EventComponent item={item} onLike={onLike} />} /> : <LoadingText />}</View>
      </FadeInView>
    </View>
  );
};
